...
fn get_traverse_dirty_ids(world:&World) -> Vec<EntityId> {
    ...
    (&parent_storage, &child_storage).children(root_id).for_each(|child| {
        update(&mut dirty_list, root_id, is_dirty, child, &parent_storage, &child_storage, &local_transform_storage, &mut dirty_transform_storage, &mut world_transform_storage);
    });
    dirty_list
}